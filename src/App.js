...
class App extends React.Component {
  ...
  async componentDidMount() {
    ...
    this.setState({ posts });
    // console.log(posts);
    this.setState({ title: posts[1].title });
    console.log(posts[1].title);
  }

  updateInput = (e) => {
    this.setState({ dreamTitle: e.target.value });
    console.log("this is: " + this.state.dreamTitle);
  };

  render() {
    console.log("test: ", this.state.posts[1]);
    return (
      <div className='App'>
        {/* logo */}
        <div className='logo' id='logo'>
        ...
        <HomePage></HomePage>
        <Page1 value={this.state.title} />
        <Page2 />
        <Page3 />
        <Page4 />
        <Q5_1 />
        <ResultPage title={this.state.title} />
      </div>
    );
  }
}
